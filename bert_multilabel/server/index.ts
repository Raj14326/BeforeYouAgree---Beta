 ...
 *  - Run clause risk analysis via the private BERT inference service.
 ...
 *  - `BERT_SERVICE_URL`, `BERT_API_KEY`, `BERT_TIMEOUT_MS`: inference service settings
 */
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/health') {
      return sendJson(response, 200, {
        ...
        source: 'tosdr',
        upstream: TOSDR_API,
        model: 'BERT (separate inference service)',
        modelReadiness: 'Check the inference service /health endpoint',
      })
    }
...
/**
 * `POST /api/analyze`: classify a document's clauses with the BERT service.
 *
 * Body: `{ content: string, serviceName?: string, documentType?: string }`.
 * Rejects empty content (400) and content over 500000 UTF-16 units (413).
 * Keeps original whitespace so finding offsets match the displayed document.
 */
async function analyzeDocument(request: IncomingMessage, response: ServerResponse) {
  const body = await readJsonBody(request)
  const content = typeof body.content === 'string' ? body.content : ''
  if (!content.trim()) return sendJson(response, 400, { error: 'Document content is required.' })
  if (content.length > 500_000)
    return sendJson(response, 413, { error: 'Document is too large to analyze.' })
  return sendJson(
    response,
    200,
    await analyzeWithBert(content),
  )
}

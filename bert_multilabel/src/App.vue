...
const findingFilters = ref<Record<string, RiskFinding['predictedLabel'] | 'needs_review'>>({})
...
function visibleFindings(termType: string) {
  ...
  return (
    analyses.value[termType]?.findings.filter((finding) =>
      filter === 'needs_review' ? finding.needsReview : finding.predictedLabel === filter) ?? []
  )
}
...
function renderDocumentView(termType: string) {
  ...
  const marks = analysis.findings
    .map((finding, index) => ({
      index,
      start: finding.predictedLabel === 'risky'
        ? (finding.start !== undefined && finding.end !== undefined &&
          finding.start >= 0 && content.slice(finding.start, finding.end) === finding.text
            ? finding.start : content.indexOf(finding.text)) : -1,
      end: 0,
              ...
              <tr v-if="retrievals[termType]">
                <td colspan="3">
                  ...
                  <div v-if="analyses[termType]" class="border rounded p-3 mb-2 bg-body-tertiary">
                      ...
                      <div class="d-flex align-items-center gap-2">
                        ...
                        >
                          ...
                          <option value="needs_review">
                            Needs review ({{ analyses[termType]?.reviewClauseCount ?? 0 }})
                          </option>
                        </select>
                      </div>
                    </div>
                    ...
                    <details v-if="analyses[termType]?.modelInfo" class="small border rounded p-2 mb-3">
                      <summary class="fw-semibold">Model and training data</summary>
                      <dl class="row mb-0 mt-2">
                        <dt class="col-sm-3">Classifier</dt>
                        <dd class="col-sm-9">{{ analyses[termType]?.model }} ({{ analyses[termType]?.modelInfo?.architecture }})</dd>
                        <dt class="col-sm-3">Base model</dt>
                        <dd class="col-sm-9">
                          <a :href="analyses[termType]?.modelInfo?.baseModelUrl" target="_blank" rel="noopener noreferrer">
                            {{ analyses[termType]?.modelInfo?.baseModel }}
                          </a>
                        </dd>
                        <dt class="col-sm-3">Training data</dt>
                        <dd class="col-sm-9">
                          <a :href="analyses[termType]?.modelInfo?.datasetUrl" target="_blank" rel="noopener noreferrer">
                            {{ analyses[termType]?.modelInfo?.trainingDataset }}
                          </a>
                          · {{ analyses[termType]?.modelInfo?.datasetLicense }}
                        </dd>
                        <dt class="col-sm-3">Scope</dt>
                        <dd class="col-sm-9 mb-0">{{ analyses[termType]?.modelInfo?.language }} · {{ analyses[termType]?.modelInfo?.scope }}</dd>
                      </dl>
                    </details>

                    <div v-if="!visibleFindings(termType).length" class="text-body-secondary small">
                      {{
                        findingFilters[termType] === 'needs_review'
                          ? 'No clauses are close to the decision threshold.'
                    ...
                    <div v-else class="risk-findings">
                      <article
                        ...
                        :key="finding.clauseId ?? `${finding.text}-${findingIndex}`"
                      ...
                      >
                        <div class="d-flex flex-wrap align-items-center gap-2 mb-1">
                          ...
                          <span v-if="finding.needsReview" class="badge text-bg-warning">
                            Needs review — close to the decision threshold
                          </span>
                          <button
                        ...
                        <p v-if="finding.riskProbability !== undefined" class="text-body-secondary small mb-0 mt-1">
                          Highest category score: {{ (finding.riskProbability * 100).toFixed(1) }}% (uncalibrated)
                        </p>
                        <div v-if="finding.categories?.some((category) => category.predicted)" class="d-flex flex-wrap gap-1 mt-2">
                          <span
                            v-for="category in finding.categories.filter((item) => item.predicted)"
                            :key="category.id"
                            class="badge text-bg-secondary"
                            :title="`Threshold ${(category.threshold * 100).toFixed(1)}%`"
                          >
                    ...
                    <p class="text-body-secondary small mb-0 mt-2">
                      Automated clause screening; unflagged clauses are not a guarantee of safety.
                      {{ analyses[termType]?.reviewClauseCount ?? 0 }} clauses need review.
                      Not legal advice.
                    </p>
                  </div>
                  <!--

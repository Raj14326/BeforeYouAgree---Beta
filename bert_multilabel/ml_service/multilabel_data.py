...
def choose_independent_thresholds(targets, scores, min_recall=0.70, beta=1.0):
    """Validation-only F-beta tuning with a recall floor and tied-score groups."""
    if not targets or len(targets) != len(scores) or not 0 < min_recall <= 1 or beta <= 0:
        raise ValueError("Invalid threshold inputs, recall floor or beta")
    thresholds = []
    objectives = []
    for j in range(len(LABEL_DEFINITIONS)):
        actual = [bool(row[j]) for row in targets]
        # A descending sweep computes the same confusion matrices as exhaustive
        # threshold evaluation, but in O(n log n), rather than O(n squared).
        ordered = sorted(((row[j], actual[i]) for i, row in enumerate(scores)), reverse=True)
        positives = sum(actual)
        tp = fp = cursor = 0
        ranked = []
        while cursor < len(ordered):
            ...
            while cursor < len(ordered) and ordered[cursor][0] == threshold:
                tp += int(ordered[cursor][1])
                fp += int(not ordered[cursor][1])
                cursor += 1
            recall = tp / positives if positives else 0
            precision = tp / (tp + fp) if tp + fp else 0
            denominator = (1 + beta * beta) * tp + beta * beta * (positives - tp) + fp
            objective = (1 + beta * beta) * tp / denominator if denominator else 0
            if recall >= min_recall and 0 < threshold < 1:
                ranked.append((objective, precision, threshold))
        if not ranked:
            raise ValueError(f"No validation threshold meets recall target for label {j}")
        selected = max(ranked)
        thresholds.append(selected[2])
        objectives.append(selected[0])
    return thresholds, {**multilabel_metrics(targets, scores, thresholds),
                        "threshold_beta": beta, "macro_fbeta": sum(objectives) / len(objectives)}

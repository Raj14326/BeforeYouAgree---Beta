...
def main():
    ...
    parser.add_argument("--risk-sampling-ratio", type=float, default=0.5)
    parser.add_argument("--threshold-beta", type=float, default=1.0,
                        help="F-beta threshold objective; beta < 1 emphasizes precision")
    parser.add_argument("--selection-metric", choices=("macro_fbeta", "macro_f1"), default="macro_fbeta",
    ...
    architecture = "LEGAL-BERT-Small" if "legal-bert-small" in args.base_model_source else "LEGAL-BERT-Base"
    ...
    if not 0 < args.risk_sampling_ratio < 1 or args.max_pos_weight < 1 or args.threshold_beta <= 0:
    ...
    # Weights must follow the actual epoch sampling prevalence. Validation
    # retains the original prevalence and never enters the sampling pool.
    ...
    sampled_positive_rate = torch.tensor([
        (risk_count / len(train)) * sum(row["targets"][j] for row in risky_rows) / len(risky_rows) for j in range(8)
    ], device=args.device)
    ...
    history, best_rank = [], (-1.0, -1.0, -1.0)
    hashes = {name: hashlib.sha256(Path(getattr(args, name)).read_bytes()).hexdigest() for name in splits}

    for epoch in range(args.epochs):
        started = time.perf_counter()
        model.train()
        safe_count = len(train) - risk_count
        risk_sample = random.choices(risky_rows, k=risk_count)
        safe_sample = (random.sample(safe_rows, k=safe_count) if safe_count <= len(safe_rows)
                       else random.choices(safe_rows, k=safe_count))
        epoch_rows = risk_sample + safe_sample
        ...
        thresholds, validation = choose_independent_thresholds(
            [r["targets"] for r in splits["validation"]], scores, args.min_recall, args.threshold_beta)
        ...
        rank = ((validation["macro_f1"], validation["micro_f1"], validation["macro_fbeta"])
                if args.selection_metric == "macro_f1" else
                (validation["macro_fbeta"], validation["macro_f1"], validation["micro_f1"]))
        if rank > best_rank:
            best_rank = rank
            ...
            manifest = {
                ...
                "model_id": f"BYA-LEGAL-BERT-{model_family}-8-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}",
                "architecture": architecture, "base_model": args.base_model_source,
                ...
                "environment": {"python": platform.python_version(), "torch": torch.__version__,
                    ...
                    "precision": "float32", "threads": args.threads},

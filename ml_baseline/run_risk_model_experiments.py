...
def calculate_clause_risk_score(risky_probability, categories):
    ...
    if categories:
        ...
        score = (0.70 * model_score) + (0.30 * category_score)
    else:
...
class MultinomialNB:
    ...
    def to_json(self, model_id, feature_mode, threshold, metrics):
        return {
            ...
            "risk_scoring": {
                "clause_score_formula": "0.70 * model_probability_score + 0.30 * category_severity_score when categories exist; otherwise 0.85 * model_probability_score",
...
def save_predictions(path, rows, probs, threshold, model_id):
    ...
    for row, prob in zip(rows, probs):
        ...
        out["predicted_binary_label"] = predict_label(prob, threshold)
